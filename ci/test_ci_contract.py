...
class EnrollmentTests(unittest.TestCase):
    def test_required_job_calls_shared_gate_with_pinned_tools(self):
        ...
        promtool = next(s for s in steps if s.get('name') == 'Install promtool')
        self.assertEqual(promtool['env']['PROMETHEUS_VERSION'], '3.5.5')
        self.assertEqual(promtool['env']['PROMETHEUS_SHA256'], '64d0beab873272b861a91df41668bc852c7e2e5b23f75c16059fb15b5630c577')
        self.assertIn('sha256sum --check --strict', promtool['run'])
        self.assertNotIn('continue-on-error', promtool)
        self.assertIn("'test_alert_contract'", (ROOT / 'ci/run_helm_tests.py').read_text())
